...
export default function News() {
  ...
  // 1. Extract category from URL (?filter=technology)
  const queryParams = new URLSearchParams(location.search);
  const initialFilter = queryParams.get('filter') || 'all';

  // 2. State Management (Removed the duplicate declaration)
  ...
  // 3. Sync state with URL changes 
  // (In case user clicks a link while already on this page)
  useEffect(() => {
  ...
  const fetchNewsData = async (cat) => {
    ...
    try {
      ...
      setError(err.response?.status === 401 ? "Session expired. Please login again." : "Failed to load news.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleImageError = (e) => {
    e.target.src = 'https://via.placeholder.com/600x400?text=News+Image+Unavailable';
  };
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-8 text-center">
            {error} <a href="/login" className="underline font-bold ml-2">Login Here</a>
          </div>
        )}

        {/* --- FEATURED STORIES (Top 2 Articles) --- */}
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {loading ? (
              ...
              articles.slice(0, 2).map((article, index) => (
                <article key={index} className="group bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col">
                  <div className="relative overflow-hidden">
                    <img 
                      src={article.image_url || 'https://via.placeholder.com/600x400?text=Premium+News'} 
                  ...
                  <div className="p-6 flex-grow flex flex-col">
                    ...
                    <div className="mt-auto flex justify-between items-center pt-4 border-t border-gray-50">
                      <div className="flex flex-col">
                        <span className="text-xs font-bold text-gray-900">{article.source_name}</span>
        ...
        {/* --- LATEST NEWS GRID (The rest of the articles) --- */}
        <section>
          <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {loading ? (
              ...
              articles.slice(2).map((article, index) => (
                <article key={index} className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col hover:border-green-200 transition-all">
                  <img 
                    src={article.image_url || 'https://via.placeholder.com/400x200?text=News'} 
                    onError={handleImageError}
                    alt="" 
                  ...
                  <div className="p-4 flex flex-col flex-grow">
                    ...
                    <div className="mt-auto flex justify-between items-center pt-3 border-t border-gray-50">
                      ...
                      <a href={article.link} target="_blank" rel="noreferrer" className="text-green-600 hover:text-green-800 font-bold text-xs flex items-center">
                        OPEN <i className="pi pi-external-link ml-1 text-[10px]"></i>
                      </a>
                    </div>
                  </div>
                </article>
        ...
        {!loading && articles.length === 0 && !error && (
          <div className="text-center py-20 bg-white rounded-2xl border-2 border-dashed border-gray-200">
            ...
            <button onClick={() => fetchNewsData(activeCategory)} className="mt-4 text-green-600 font-bold">Try Refreshing</button>
          </div>
        )}
        ...
        <div className="text-center mt-16">
          <button 
            ...
            disabled={loading}
            className={`bg-white border-2 border-gray-900 text-gray-900 px-10 py-3 rounded-full font-bold hover:bg-gray-900 hover:text-white transition-all transform active:scale-95 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
